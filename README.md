...
A monotonic stack can be used to solve the problem. One important thing to realize when building the tree is that each number should be inserted as the right child of the the first larger number at its left, and if the larger number already has a right child, its right subtree should be the left subtree of current number. Therefore, when processing each of the numbers, we compare it to the top element of the stack. If current number is larger, the top is popped out and set to be the left child of current number. However, this process will be repeated until the stack is empty or the top element is larger, so the final left child of current number would be the last element in the stack that is smaller. After the repetition, if there are still elements in the stack, current number is set to be the right child of the top element. The stack would be monotonic and in descending order, meaning that the bottom of the stack would be the root node.  
